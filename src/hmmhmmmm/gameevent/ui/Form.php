...
use hmmhmmmm\gameevent\libs\xenialdan\customui\event\UICloseEvent;
use hmmhmmmm\gameevent\libs\xenialdan\customui\event\UIDataReceiveEvent;
use hmmhmmmm\gameevent\libs\xenialdan\customui\elements\Button;
use hmmhmmmm\gameevent\libs\xenialdan\customui\elements\Dropdown;
use hmmhmmmm\gameevent\libs\xenialdan\customui\elements\Input;
use hmmhmmmm\gameevent\libs\xenialdan\customui\elements\Label;
use hmmhmmmm\gameevent\libs\xenialdan\customui\elements\Slider;
use hmmhmmmm\gameevent\libs\xenialdan\customui\elements\StepSlider;
use hmmhmmmm\gameevent\libs\xenialdan\customui\elements\Toggle;
use hmmhmmmm\gameevent\libs\xenialdan\customui\windows\CustomForm;
use hmmhmmmm\gameevent\libs\xenialdan\customui\windows\ModalForm;
use hmmhmmmm\gameevent\libs\xenialdan\customui\windows\SimpleForm;
...
class Form{
   ...
   public function Menu(Player $player, string $content = ""): void{
      ...
      $form = new SimpleForm(
         $this->getPrefix()." Bảng điều khiển",
         $content
      );
   ...
   public function Create(Player $player, string $content = ""): void{
      $form = new CustomForm(
         $this->getPrefix()." Tạo sự kiện"
      );
      $form->addElement(new Label($content));
      $form->addElement(new Input($this->getPlugin()->getLanguage()->getTranslate("form.create.input1"), "Thợ săn quái vật"));
      $form->addElement(new Input($this->getPlugin()->getLanguage()->getTranslate("form.create.input2"), "600 giây = 10 phút"));
      $form->addElement(new Input($this->getPlugin()->getLanguage()->getTranslate("form.create.input3"), "Tiêu diệt 100 thây ma"));
      $form->addElement(new Input($this->getPlugin()->getLanguage()->getTranslate("form.create.input4"), "64 viên kim cương"));
      $form->addElement(new Input($this->getPlugin()->getLanguage()->getTranslate("form.create.input5"), "tp {player} ~ ~ ~"));
   ...
   public function Edit(Player $player, string $eventName, string $content = ""): void{
      ...
      $form = new SimpleForm(
         $this->getPrefix()." Chỉnh sửa ".$eventName,
         $content."\n".$text
      );
      $array = [
   ...
   public function SetTime(Player $player, string $eventName, string $content = ""): void{
      $form = new CustomForm(
         $this->getPrefix()." Đặt thời gian ".$eventName
      );
      $form->addElement(new Label($content));
      $form->addElement(new Input($this->getPlugin()->getLanguage()->getTranslate("form.create.input2"), "600"));
