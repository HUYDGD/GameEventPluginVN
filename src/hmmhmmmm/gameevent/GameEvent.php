...
use hmmhmmmm\gameevent\libs\xenialdan\customui\API as XenialdanCustomUI; //?
...
class GameEvent extends PluginBase implements GameEventAPI{
   ...
   private $prefix = "?";
   private $facebook = "§ckhông có";
   private $youtube = "§ckhông có";
   private $discord = "§ckhông có";
   ...
   public function onEnable(): void{
	  ...
	  $this->getLogger()->info("§aGameEvent[việt hóa] v3.2 đã được bật!");
      ...
      if(!in_array($langConfig, $this->langClass)){
         $this->getLogger()->error("§cKhông tìm thấy ngôn ngữ ".$langConfig.", hãy thử các ngôn ngữ khác như: ".implode(", ", $this->langClass));
   ...
   public function setEnabledEvent(string $name, bool $enabled = false): void{
      $this->getServer()->broadcastMessage($this->getPrefix()." ".($enabled ? $this->getLanguage()->getTranslate("enabledevent.on", [$name]) : $this->getLanguage()->getTranslate("enabledevent.off", [$name])));
      $eventData = $this->getData();
      $data = $eventData->getAll();
   ...
   public function getInfoStartEvent(string $event): string{
      ...
      if(isset($data["event"]) && isset($data["event-present"])){
         ...
         if($data["event"][$name]["enabled"]){
            $text = $this->getLanguage()->getTranslate("infostartevent.on", [$name, $data["event"][$name]["info"], $this->sendTime($data["event"][$name]["time"]), $data["event"][$name]["info-award"]]);
         }else{
   ...
   public function getInfoAwardEvent(string $event): string{
      ...
      if(isset($data["event"]) && isset($data["event-present"])){
         ...
         if($data["event"][$name]["enabled"]){
            $text = $this->getLanguage()->getTranslate("infoawardevent.on", [$name, $data["event"][$name]["info-award"]]);
         }else{
