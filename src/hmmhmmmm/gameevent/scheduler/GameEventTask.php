               ...
               if($text[0] == $this->getPrefix()){
                  if(isset($data["event"]) && isset($data["event-present"])){
                     if(count($data["event"]) == 0){
                        $sign->setText($this->getPlugin()->getLanguage()->getTranslate("event.without"), "", "");
                     }
                     $event = TF::clean($text[1]);
                     $event = explode(" ", $event); 
                     $event = $event[1];
                     if(!isset($data["event"][$event])){
                        $sign->setText($this->getPlugin()->getLanguage()->getTranslate("event.notfound", [$event]), "", "");
                        return;
                     }
                     ...
                     $sign->setText($eventName, $awardInfo, $time);
                  }else{
                     $sign->setText($this->getPlugin()->getLanguage()->getTranslate("event.without"), "", "");
                  }
               }
      ...
      if(isset($data["event"]) && isset($data["event-present"])){
         ...
         if($data["event"][$event]["enabled"]){
            ...
            if($data["event"][$event]["time"] <= 10){
               $this->plugin->getServer()->broadcastMessage($this->getPlugin()->getLanguage()->getTranslate("scheduler.runevent.willend", [$event, $this->plugin->sendTime($data["event"][$event]["time"]), $eventNew]));
            }
            if($data["event"][$event]["time"] <= 0){
               ...
               $this->plugin->getServer()->broadcastMessage($this->getPlugin()->getLanguage()->getTranslate("scheduler.runevent.end", [$event]));
               if(isset($data["event-next"])){
